...
export class WellcomeComponent implements OnInit {
  ...
  constructor(private router:Router, private pageSubject: PagesService) {}
  

  ngOnInit() {
    
    //let pt = decodeURIComponent(window.location.pathname).slice(1) === '' ? true : false;
    let id = this.pageSubject.getCurrentUrl()["id"];
    let currentUrl = this.pageSubject.getCurrentUrl()["currUrl"];
   
    //animate wellcome component
    if(id > 1 && currentUrl === "/") $('.carousel').carousel();
    $(function(){
  ...
  //ngOnDestroy() {
    //if(this.urlSr.getUriObj()) this.urlSr.getUriObj().unsubscribe();
  //}
}
